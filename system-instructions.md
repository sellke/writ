...
- **Allowed values:** `orchestration`, `capability`, or a reserved negative ordinal offset (`-N`) — regex `^(orchestration|capability|-[0-9]+)$`.
...
**Reserved ordinal offsets (`-1`, `-2`, ...) are reserve-only.** They are documented as a future N-step resolution mechanism but resolve today only as a 2-band clamp: any negative offset lands on the same floor as `capability` (or inherit, if the platform exposes just one band). No consumer may depend on finer-grained ordinal behavior yet.

> **Review trigger: 2026-10-16** (90 days post-ship). If no adapter has built N-step (>2-band) resolution by this date, deprecate or revisit the ordinal-offset reservation. Mirrors the `required_skills:` review discipline above.
