...
**Status: documented, and the convention still has no consumer.** The 2026-08-03 review trigger (90 days post-ship, per the foundation spec `2026-05-03-skills-foundation`) resolved **revisit → adopt** on a justification that consisted almost entirely of a named future consumer: Phase 10 progressive disclosure ([ADR-021](.writ/decision-records/adr-021-progressive-disclosure-token-budget.md)). **That consumer evaluated the mechanism and did not adopt it.**

The reason is measured, not stylistic: `required_skills:` is an **eager pre-load**. The harness loads every declared skill *before any phase work begins* (see **Harness contract** above; [`adapters/claude-code.md`](adapters/claude-code.md) says the same in its Skills → Invocation section), and selection is per **command**, never per **run** — a static array cannot express "only what this invocation needs". So extraction under this field moves the extracted bytes into the **floor**, where every invocation pays them, and a disclosed command costs *more* per invocation than the monolith it replaced. Phase 10 loads its skills with an inline `Read skills/<name>/SKILL.md` at the point of need instead, which is genuinely conditional. ADR-021's 2026-08-12 amendments carry the full record.

The schema above is unchanged and stays documented: it is referenced by `/new-skill`, implemented in all three adapters, and validated by `scripts/eval-leanness.py`'s `check_required_skills()`. Deprecating it is an ADR-scale decision and belongs to the review below, not to the spec that discovered the mechanism was wrong for one phase.

**Review trigger: 2026-11-11.** Restored rather than left settled — a resolution whose premise turned out to be false should not survive as a settled adoption, which is how a convention accretes permanence it never earned. The date is aligned to ADR-021's own review trigger, not a fresh 90-day clock: that review already reads the per-invocation load data that would justify a consumer, so it is the reading best placed to answer whether an eager mechanism ever acquired one. Two triggers, one reading. **Terms:** if no command or agent declares `required_skills:` by that date, deprecate; if one does, record it and reset the clock.
