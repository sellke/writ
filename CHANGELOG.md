...
- **`/create-uat-plan` command** — New standalone command that generates a structured UAT (User Acceptance Test) plan from completed spec stories. Reads acceptance criteria, error maps, shadow paths, edge cases, and experience design from spec packages, then produces human-readable test scenarios with preconditions, steps, expected results, and pass/fail checkboxes. Enriches scenarios with concrete implementation details from "What Was Built" records. Handles partial completion (generates scenarios only for completed stories, notes pending ones). Output lives at `.writ/specs/{spec}/uat-plan.md`. Supports `--check` mode for preview without writing. See `commands/create-uat-plan.md`.

- **Context routing in `/implement-story`** — The orchestrator now parses context hints from story files (`## Context for Agents`), fetches referenced content from spec.md/technical-spec.md, extracts agent-specific spec-lite sections, and routes targeted context to all 5 pipeline agents. Architecture check and coding agents receive the "For Coding Agents" section; review agents get "For Review Agents"; testing agents get "For Testing Agents"; documentation agents get full spec-lite. Coding agents also receive "What Was Built" records from completed dependencies via new `dependency_wwb_context` parameter. Graceful degradation for missing content, legacy specs, and missing sections. See `commands/implement-story.md` Step 2 (items 4-5).
