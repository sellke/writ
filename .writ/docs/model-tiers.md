...
`scripts/lint-skill.sh` validates any declared `model_tier` value — in skill frontmatter, an agent's Agent Configuration block, or a command's prose note — against the shared allow-list (`^(orchestration|capability)$`), whenever it's pointed at a file. It does not add a new automated sweep over `agents/*.md` or `commands/*.md` by default; that remains a deliberate, explicit choice per invocation.
