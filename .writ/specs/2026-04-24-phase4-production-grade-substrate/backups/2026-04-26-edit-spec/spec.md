...
5. **Agent on a fresh task can find and load relevant knowledge entries without the maintainer prompting it** (ADR-005 success criterion). Validated by dogfooding: ship a Story 6+ feature where the agent's prompt does not mention `.writ/knowledge/` and verify a relevant entry was loaded.
...
| Knowledge ledger entries written but never loaded by agents (ADR-005 named risk) | Medium | High | Story 1's Definition of Done includes dogfood validation: ship a follow-up feature where the agent loads a backfilled entry without prompt-side mention |
...
- All 5 stories marked `Completed ✅`
- `bash scripts/gen-skill.sh --dry-run && git diff --exit-code SKILL.md` exits 0
- `bash scripts/eval.sh` exits 0 against the full Writ surface
- `.writ/knowledge/` contains ≥5 backfilled entries across ≥2 categories
- CI runs `gen-skill.sh --check` and `eval.sh` on every PR
- All three adapter docs (`cursor.md`, `claude-code.md`, `openclaw.md`) reference the preamble convention and the knowledge-loading hook
- The PR shipping this spec includes a drift log (per the standard pipeline)
- A 90-day review is scheduled (per ADR-005 review trigger) against the success criteria above
