...
- Step 4: **one hit — line 343.** `.writ/product/roadmap.md` still restates the false claim, inside the backlog item describing this very spec. `.writ/product/` is on the active surface per Business Rule 3, so this is a genuine open item, not a pass. It is recorded rather than fixed because the roadmap belongs to the sibling spec `2026-08-11-component-contract`. See Discrepancy 1 — that sibling's Story 1 does **not** claim this line.
...
1. **`.writ/product/roadmap.md:343` still carries `verified 0/31 files`, and no spec currently claims it.** `.writ/product/` is on the active surface per Business Rule 3, so the retired literal survives on the active surface in exactly one place. Story 1 recorded this as a finding rather than repairing it, correctly — the roadmap belongs to the sibling spec `2026-08-11-component-contract`. But that sibling's Story 1 only claims roadmap lines 316 and 330 (the `## Completion` mandate) and its Revision Log row; line 343 appears in none of its acceptance criteria. **The line is unowned.** The sibling is also still unbuilt (all its story checkboxes are `- [ ]`), so this cannot be closed by waiting for it. This is the one open item from the spec worth tracking.
