...
- [x] Given no implemented headless driver (or no `pnpm`) is on `PATH`, when `pipeline-baseline.py run` is invoked for a model that has a driver, then it exits non-zero naming the missing binary and its install path, and no `$TMPDIR/writ-baseline-*` directory, no baseline-file write, and no `git` or driver subprocess occurs. Absence of any vendor API key is not a refusal. A model with no headless driver (Grok, local weights, an IDE session) is refused with an `ingest` instruction, not a missing-key error `[AC-4.1]`
...
3. **Task 4.2 smoke run deferred.** Originally deferred because `ANTHROPIC_API_KEY` was unset. **2026-09-07:** a resident API key is no longer required (see Approved Scope Addition on `spec.md`); smoke/live runs use the operator's `claude` login. What 4.2 must confirm: slash command resolves with the path argument; the permission flags let Bash run; `result` event field names for `usage`/`total_cost_usd`/`subtype`; gate verdicts arrive as `Task` tool_results; `pnpm install --frozen-lockfile` wall time; `--model claude-fable-5-1` is accepted (record `model_resolved`); `apiKeySource`.

4. **2026-09-07 — no resident key; driver ≠ model (user-approved).** `run` preflight no longer requires any vendor API key. `--driver auto` selects a headless CLI from the model id; Grok / local / IDE-session models use `ingest`. AC-4.1 amended.
...
- **Error map rows:** [No vendor key → `run` does not refuse; missing driver or `pnpm` → stop before any checkout, name install path; model with no headless driver → `ingest`; headless run timeout → `status: timeout`, continue] — spec.md → ## 🎯 Experience Design → Error experience; spec-lite.md → ## For Testing Agents → Shadow Paths → Nil input, Upstream error
...
Task 4.2 is deferred (user decision 3). A maintainer with `claude` and `pnpm` on `PATH` (logged in via the operator CLI — no Writ-resident key) runs, from this repo's root:

```bash
python3 scripts/pipeline-baseline.py run \
