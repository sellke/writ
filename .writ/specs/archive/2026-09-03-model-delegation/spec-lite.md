...
- *(DEV-009–013, Story 5)* ⚠️ Medium: the entry notice's positive path is **not observable on Cursor today** — every listed slug self-assesses at or above `high` and the harness prompt states no effort (nil-origin skip unless the `Task`-slug origin source is used); bite is on Claude Code (`haiku`) and Codex (`low`); no ranking added (Business Rule 9). `entry_level` presence is checked frontmatter-scoped (`frontmatter_has_key()`, own `entry-level` `CHECKS` entry) because `new-command.md:161` carries a pinned fenced scaffold literal; AC-5.1's count is frontmatter-scoped for the same reason. `lint-skill.sh` routes `commands/[!/]*.md` (basename not `SKILL.md`) to ADR-024 value checks only. Value-grammar fixtures live in Story 1's `test_lint_model_tier.sh`; Story 5's test holds the real-tree run and routing.
- *(DEV-014–015, Story 4)* Step 2.6a runs `ac-trace.py check --spec <folder> --repo <folder>` (folder-scoped `--repo`, unlike `/verify-spec`); the `escalated` site label is ADR-024's `create-spec.2.6`, never the `2.6a` heading, and the eval pin extends through `, origin=`; `test_governor_enforcement.py`'s `KNOWN_OVER_BUDGET` was re-pinned in-story with dated disclosure (both escalation commands plus Story 5's four `entry_level:` overages) — no budget recovery.
