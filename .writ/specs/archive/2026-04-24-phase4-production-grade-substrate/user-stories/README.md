...
|---|-------|--------|----------|--------|-------|----------|
| 1 | [Knowledge Ledger v1](story-1-knowledge-ledger.md) | Completed ✅ | High | S–M (~2–4 days) | 8 | 8/8 |
...
| 5 | [Eval Tier 1 (Static Checks)](story-5-eval-tier-1.md) | Completed ✅ | High | S (~1 day + 0.5 day triage) | 8 | 8/8 |

**Total Tasks:** 37 / 37 complete. Two organic post-ship validations are tracked as issues, not as open tasks: Story 1 knowledge-loading confirmation and Story 5 remote CI gate confirmation (see "Validation Plan" below).
...
> **Phase-level entry condition (per roadmap):** Phase 1 features dogfooded and stable enough to build on. Independent of story-level dependencies.
...
- **Story 1:** Implementation reviewed locally (knowledge-loading hook wired through orchestrator and three agents). Organic load-confirmation tracked at [`2026-04-26-story-1-knowledge-loading-organic-validation`](../../../issues/improvements/2026-04-26-story-1-knowledge-loading-organic-validation.md); confirms naturally on the next Phase 5 feature.
- **Story 2:** This very spec shows `Owner: @adam` in frontmatter and is surfaced correctly by `/status` and `/verify-spec`.
- **Story 3:** Deliberate manifest-drift simulation locally caused `gen-skill.sh --check` to exit 1; reverted post-verification.
- **Story 4:** Manual smoke test under Cursor confirmed preamble loads via static reference; eval Tier 1's `preamble` check enforces the convention going forward.
- **Story 5:** Local violations cause `eval.sh` to exit 1 with file:line findings; `--fix` for `preamble` was confirmed end-to-end. Remote CI gate confirmation tracked at [`2026-04-26-story-5-remote-ci-gate-organic-validation`](../../../issues/improvements/2026-04-26-story-5-remote-ci-gate-organic-validation.md); confirms naturally on the first PR through CI.
