...
- [x] Given the same command file, when default-path spawn sites and the Pipeline table `Runs as` / `Skipped in` columns are rewritten, then Gate 1 and Gate 3 on the default path name only `coding-agent` and `evaluator-agent`; completeness and worktree-integration notes that list spawn gates name only those two on default; Gate 3 `Runs as` is `evaluator-agent` on default and `review-agent` on `--full-pipeline`; architecture-check, testing, visual-qa, and docs spawn only on `--full-pipeline` (visual-qa still also skipped with no visual refs); and `--full-pipeline` still names architecture-check, review, testing, visual-qa, and documentation. `[AC-2.2]`
- [x] Given a per-story `evaluator_fail_count` starting at 0, when the evaluator FAILs once, then the command recodes via Gate 1 without AskQuestion; when a second consecutive evaluator FAIL increments the count to 2, then it prints one notice and treats the remainder of this story as `--full-pipeline` without AskQuestion; `--quick` never escalates (no evaluator); and `--review-only` FAIL ends the run with no recode spawn and no silent `--full-pipeline`. `[AC-2.3]`
- [x] Given Stage 2b’s FAIL-only override still after Gate 3, when the Gate 3 agent returns, then `review-override.py` is still invoked; the residual sentence on the default path names `evaluator-agent` (and still names `review-agent` on `--full-pipeline`); script `fail` recodes and script `pass` does not wash an evaluator FAIL; and on default a `test-integrity.py` `fail` uses BLOCKED escalation with agent `coding-agent` restarting Gate 1, while `--full-pipeline` keeps `testing-agent` restarting Gate 4. `[AC-2.4]`
- [x] Given Gate 0 / 4.5 / 5 bodies and `gates:` frontmatter, when the default path runs, then Gate 0 / 5 run the script only (`arch-check.py` / `docs-check.py`), Gate 4.5 is skipped (no visual-qa spawn even with visual refs), Gate 0 ADR-024 floor→anchor ABORT re-run and ABORT confirmation are `--full-pipeline` only, Gate 3 heading stays (no new gate numbers), `gates:` provenance is unchanged (`gate3_review` still `script: scripts/review-override.py`), and this story does not edit `scripts/review-override.py`, `scripts/eval.sh`, or `agents/review-agent.md`. `[AC-2.5]`

## Implementation Tasks

- [x] 2.1 Write `scripts/tests/test_implement_story_default_path.py` (pytest, Python 3.9) that reads `commands/implement-story.md` as text and asserts: no `--default` token; Invocation table contains `--full-pipeline`; no-flag / `--quick` / `--review-only` spawn claims match the matrix; default-path `> **Agent:**` (or equivalent spawn) names only `coding-agent` and `evaluator-agent`; `--full-pipeline` still names architecture-check, review, testing, visual-qa, documentation; two-fail escalation prose exists and does not AskQuestion; Gate 4 default fail names `coding-agent` / Gate 1; `review-override.py` is still invoked; `gates:` frontmatter provenance is unchanged (`gate3_review` still `script: scripts/review-override.py`) `[AC-2.1, AC-2.2, AC-2.3, AC-2.4, AC-2.5]`
- [x] 2.2 Rewrite the Invocation table and Pipeline table in `commands/implement-story.md` per `technical-spec.md` → `## 2. Default path + flags` → `### Invocation` and `spec.md` → `## Detailed Requirements` → `### Story 2 — Default path + flags` (no `--default`) `[AC-2.1, AC-2.2]`
- [x] 2.3 Update default-path spawn sites (Gate 1 / Gate 3), completeness and worktree-integration notes, and the Gate 3 body: spawn `evaluator-agent` on default, skip on `--quick`, evaluator-only on `--review-only`, `review-agent` on `--full-pipeline`; keep the Stage 2b `review-override.py` invoke; residual sentence names `evaluator-agent` on default only `[AC-2.2, AC-2.4]`
- [x] 2.4 Add two-fail escalation (`evaluator_fail_count`, first FAIL → Gate 1 recode, second FAIL → one `--full-pipeline` notice, no AskQuestion) and rewrite the Gate 4 default `test-integrity.py` `fail` BLOCKED template to `coding-agent` / restart Gate 1 (`--full-pipeline` keeps `testing-agent` / Gate 4) `[AC-2.3, AC-2.4]`
- [x] 2.5 Rewrite Gate 0 / 4.5 / 5 default bodies to script-only / skip; leave Gate 0 ABORT confirmation and ADR-024 floor→anchor re-run as `--full-pipeline` only; do not change `gates:` provenance; do not edit `scripts/review-override.py`, `scripts/eval.sh`, or `agents/review-agent.md` `[AC-2.5]`
- [x] 2.6 Verify acceptance criteria: `uv run --python 3.9 pytest scripts/tests/test_implement_story_default_path.py` green; confirm the locked matrix, two-fail notice, Gate 4 default recode, FAIL-only override invoke, and unchanged `gate3_review` script line `[AC-2.1, AC-2.2, AC-2.3, AC-2.4, AC-2.5]`
...
1. **`scripts/tests/test_implement_story_default_path.py`** (288 lines)
   - Text contract for AC-2.1–2.5 plus pin-sensitive literals (routing table, Gate 3 heading, `gates:` provenance, ADR-024 strings, no `--default`). JS-style authenticity pin for `commands/implement-story.md`.

### Files Modified

- **`commands/implement-story.md`** (34063 bytes, overage still 9103)
  - Default path is `coding-agent` + `evaluator-agent` + Stage 2b scripts. `--full-pipeline` is the six-agent hatch. Two-fail escalation. Gate 4 default fail recodes via `coding-agent` / Gate 1. Gate 0 / 4.5 / 5 default script-only / skip.
- **`spec-lite.md`** — Small-drift auto-amend: Gate 0.5 is default inline, not `--full-pipeline`-only; `--review-only` FAIL ends the run.
- **`drift-log.md`** — created with DEV-001 and DEV-002.

`scripts/review-override.py`, `scripts/eval.sh`, and `agents/review-agent.md` left unchanged.

### Implementation Decisions

1. **Byte-neutral rewrite** — file stayed exactly 34063 bytes so `KNOWN_OVER_BUDGET` 9103 still matches. Substitute prose; no duplicated gate bodies.
2. **Keep heading `#### Gate 3: Review Agent`** — existing pytest / eval.sh pins slice on that literal.
3. **`--full-pipeline` Agent markers** sit on the line after a `--full-pipeline` guard so default-path `> **Agent:**` names only coding-agent and evaluator-agent.
4. **Reset `evaluator_fail_count` on PASS** so a later isolated FAIL is a first fail, not an instant hatch.

### Test Results

**Verification:** Automated
- ✅ `uv run --python 3.9 pytest scripts/tests/test_implement_story_default_path.py` — 9 passed
- ✅ Regression wiring / override / governor / provenance — 118 passed (127 total)
- ✅ `test-integrity.py authenticity` — pass
- ⚠️ `test-integrity.py coverage` — unverifiable (`no_coverage_report`)
- Mechanical: arch-check `pass` (rederived proceed; agent CAUTION); review-override `unverifiable`; drift-format `pass`; docs-check `unverifiable`; build-smoke `unverifiable`

**Coverage:** N/A (markdown command + contract test; no new production Python)
...
- **Drift:** Small
- **Security:** Clean
- **Boundary Compliance:** Owned files only; protected Readable files untouched.

### Deviations from Spec

- **[DEV-001] Gate 0.5 leftover “full pipeline” wording** — Severity: Small
  - Spec said: Gate 0.5 is inline on default; skip `--quick` / `--review-only` / `/prototype`
  - Reality: Prototype note still says “full pipeline”
  - Resolution: Auto-amended
  - Spec amendment: spec-lite Implementation Approach
- **[DEV-002] Control flow omits `--review-only` FAIL exception** — Severity: Small
  - Spec said: `--review-only` FAIL ends the run
  - Reality: Invocation + Gate 3 state it; Control flow is the general recode rule
  - Resolution: Auto-amended
  - Spec amendment: spec-lite Error Handling

### Next Story

**Story 3:** spawn-cap.py, eval.sh check, adapter wording, spawn-cap proof.
