...
so the story can later be mapped to its exact commit without guessing. Revert
tooling treats this field as its **highest-confidence resolution layer**; the
other layers are inference. The SHA does not exist until the commit does, so it
...
4. **Land the field in the next commit.** Fold the one-line header write into
   ...
   **Do not `--amend` the completion commit; amending would rewrite the SHA
   just recorded.** The recorded SHA must point at the completion commit;
   the record-SHA commit itself is inert.

### Backward compatibility

The field is **optional**. Work completed before this convention, or by reduced
runs that skip bookkeeping, lacks `> **Commit:**`. Resolution tooling
