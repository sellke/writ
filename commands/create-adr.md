---
name: create-adr
description: "Record an architecture decision with alternatives analysis and consequences, negative ones included. For high-switching-cost or convention-breaking choices, not routine technical picks."
...
Create Architecture Decision Records that document significant architectural choices with rationale, alternatives analysis, and consequences, negative ones included. An ADR without an alternatives comparison is a post-hoc justification.
...
Before proceeding, check whether relevant research exists.
...
Output: a problem statement and the decision's blast radius.

---

### Step 2: Define Scope & Evaluation Criteria

Scope the ADR to one architectural question.
...
Force-rank the top 3 drivers that will tip the decision.
...
Spend the most time on the "considered options" section; it is what prevents future teams from re-litigating settled decisions.
...
| **Consequences** | Positive and negative consequences. Every decision has downsides; document them. Include mitigation strategies for each negative consequence. |
...
**Record dissent.** If stakeholders disagreed, document the dissenting view and how it was addressed.

**The test of a good ADR:** A new team member reads it in 5 minutes and understands what was decided, why, and what was rejected and why.
...
**Technical ADRs (sequential from the next available number):** Created via `/create-adr` for architectural, infrastructure, implementation, and tooling decisions. They continue the numbering sequence after any product-level ADRs already in place.
...
**Migration note:** If your project has a `.writ/product/decisions.md` from an earlier `/plan-product` run, it is not modified, migrated, or deleted by these instructions. You may create ADRs to formalize the decisions it documents; migration is optional and unscripted.
...
2. **Alternatives documented** — the ADR contains at least three considered options (including status quo) with pros, cons, and effort estimates
