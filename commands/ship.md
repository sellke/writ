...
`/ship --test --recommend` is an internal single-spec continuation with a valid
`delivery_context`; `--recommend` without `--test` or with `--draft`,
`--dry-run`, `--no-split`, or `--rebase` fails before mutation. When
`--recommend` is absent, the ordinary command below is authoritative and
unchanged.

Recommended shipping uses mandatory test semantics: a failed suite blocks and
cannot be shipped as draft. Reconcile the current branch, remote/base/head, test
artifact, and existing commits before changing git. The test artifact is safe
repo-local structured JSON containing schema, nonempty command, `exitCode: 0`,
`status: passed`, and exact full current head; return its exact SHA-256. Select commit grouping from
the existing heuristic using observable diff/layer/buildability evidence,
append a Pending recommendation entry, persist its decision/operation key, then
create only the missing commits. Preserve unrelated dirty paths exactly; mixed
ownership or a hook-altered unowned path blocks.

Before returning, finalize that same entry. `decision_id` must equal
`audit_entry_id`, and its Decision/Result must state the chosen strategy,
ordered commit SHAs, and exact resulting head. Verify every listed SHA exists as
a commit in this repository, each is an ancestor of the next, and the final SHA
equals both git `HEAD` and persisted current head. `single` lists exactly one
commit; `split` lists at least two. Unrelated/nonexistent SHAs, unbound IDs, or
strategy/result contradictions block.

Before PR creation, return control to implement-spec's provider protocol:
lookup by provider repository/base/head must precede creation. `/ship` may push
only after comparing the configured remote head and must never force push. A
non-fast-forward, authentication, or authorization denial is reported once and
stops. It never merges.

Return:

```yaml
schema: recommend-ship-result-v1
execution_id: string
mode: recommend
...
test:
  ...
  command: string
  exit_code: 0
  evidence_artifact: repo-relative-path
  evidence_artifact_sha256: lowercase-hex
  evidence_head_sha: full-sha
commits:
  strategy: single | split
  grouping_basis: single-file | under-50-lines | tightly-coupled | logical-layers-buildable
  decision_id: string
  audit_entry_id: REC-NNN
...
pr_lookup:
  ...
  required_next_operation: findPullRequest
```

The parent persists this normalized evidence with `record-ship`; PR lookup and
the at-most-one `createPullRequest` mutation remain under implement-spec's
