...
check_git_notes_audit() {
  ...
  require_literal "$install" '+refs/notes/writ:refs/notes/origin-writ' "install.sh must fetch notes into the remote-tracking ref, not onto refs/notes/writ."
  require_literal "$ship" 'refs/notes/origin-writ' "/ship must merge the fetched remote notes before attaching."
  require_literal "$release" 'refs/notes/origin-writ' "/release must merge the fetched remote notes before attaching."
  require_literal "$ship" 'git push origin refs/notes/writ' "/ship must push the notes ref — an unpushed note is local-only."
  require_literal "$release" 'git push origin refs/notes/writ' "/release must push the notes ref — an unpushed rollup is local-only."
